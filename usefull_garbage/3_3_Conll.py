...
def extract_triple(L, AdjL, trigName, ID, Frames, df, output):
	StopWords = [u"être","avoir","pas","ne","l","trop","plus","beaucoup","aussi","encore","faire","aller","venir", u"très","que","tout",
		    "devoir","rendre",u"même","non","ainsi","falloir","juste"] 
	if L[ID][7] != 'root':
		parent = int(L[ID][6]) 
...
#					df[i].write('\n\n\n')
			...
			if (A[3] in ['V','N','A']) and (C[3] in ['V','N']): 
				for effet, pos in zip(A[2].split('~'), A[4].split('~')):
					if (effet not in StopWords) and (pos != 'ADV'):
						for cause, pos in zip(C[2].split('~'), C[4].split('~')):
							if (cause not in StopWords) and (pos != 'ADV'):
								if effet[:2] == "l'":
									print "fkhefr"
									effet = effet[2:]
								if cause[:2] == "l'":
...
#		File[key].append(open("./text/text_" + key + "_triple.txt", 'w', 'utf-8'))
	...
	outputFile = open("./training_data.txt",'w', 'utf-8')
